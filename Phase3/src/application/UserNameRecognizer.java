...
public class UserNameRecognizer {
	...
	public static String checkForValidUserName(String input) {
		...
		if(input.length() <= 0) {
			...
			return "\n*** ERROR *** The input is empty. If you'd like to test a username, please input a series of letters, numbers, or symbols.";
		}
		...
		while (running) {
			...
			case 0: 
				...
				if ((currentChar >= 'A' && currentChar <= 'Z' ) ||		// Check for A-Z
					    ...
					    // line 116 was removed to exclude numbers being used as the first character in the userName
			  ...
			  // Check for a-z
				...
				// If it is none of those characters, the FSM halts
					...
					//return "\nUsernames can only begin with a letter!"; //Helpful error message that informs the user that they input an unlawful username 					
					running = false;
			...
			case 1: 
				...
				// . -> State 2
				else if (currentChar == '.' || currentChar == '_' || currentChar == '-') {	// Check for symbols: '.', '_', '-"
					//I added the symbols '_', '-' as per the program requirements 
					nextState = 2;
		...
		case 2:
			...
			userNameRecognizerErrorMessage +=
				"A UserName character after a period, dash, or underscore must be A-Z, a-z, 0-9.\n"; //error message stating that there cannot be any periods, dashes, or underscores in a row. 
			return userNameRecognizerErrorMessage;
